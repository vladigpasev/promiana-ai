...
function Post({ id, title, description, author, tags, image }: PostsProps) {
    const tagList = tags.split(',').map(tag => tag.trim());

    return (
        <div className='h-full'>
            <div className="card w-[100%] bg-base-100 shadow-xl h-full">

                <figure>
                    <img src={image ? image : '/opengraph-image.png'} alt={`${title} Image`} />
                </figure>
                <div className="card-body">
                    ...
                    <div className="card-actions justify-end mt-5">
                        {tagList.map((tag, index) => (
                            <div key={index} className="border-black border rounded-3xl text-sm px-2"># {tag}</div>
                        ))}
                    </div>
